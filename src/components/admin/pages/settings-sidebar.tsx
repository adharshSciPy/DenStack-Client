...
import labBaseUrl from "../../../labBaseUrl.js";
// Types
...
// Add these interfaces near your other types
interface LabVendor {
  id: string;
  name: string;
...
export default function SettingsGrid() {
  ...
  const clinicId =
    useSelector((state: any) => state?.auth?.clinicId) ??
    useSelector((state: any) => state?.auth?.user?.clinicId) ??
    useSelector((state: any) => state?.auth?.user?.id) ??
    null;
  ...
  const [activeSettingsTab, setActiveSettingsTab] = useState<
    "grid" | "practice" | "whatsapp"
  >("grid");
  const [activeProcedureTab, setActiveProcedureTab] = useState<string>(
  ...
  const [labForm, setLabForm] = useState<LabVendorFormData>({
    name: "",
    contactPerson: "",
    email: "",
    services: [],
  ...
  const getActiveTabConfig = () => {
    return (
      tabConfigs.find((tab) => tab.key === activeProcedureTab) || tabConfigs[0]
    );
  };
  ...
  const fetchClinicProfile = async () => {
    try {
      const res = await axios.get(
        `${clinicServiceBaseUrl}/api/v1/auth/clinic/view-clinic/${clinicId}`,
        {
  ...
  const fetchItems = async (page = 1, search = "") => {
    try {
      ...
      if (
        ...
        alert(
          `Failed to fetch ${getActiveTabConfig().title.toLowerCase()}. Please try again.`,
        );
      }
    } finally {
      setIsLoading(false);
  ...
  const fetchWhatsAppSettings = async () => {
    try {
      ...
      const response = await whatsappApi.get(
        `api/v1/whatsapp/settings/${userId}`,
      );
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setVerificationMessage(
          "❌ Verification failed. Please check your Phone Number ID.",
        );
      }
    } catch (error: any) {
  ...
  const fetchMessageHistory = async (page = 1) => {
    try {
      ...
      const response = await whatsappApi.get(
        `api/v1/whatsapp/history/${userId}?${params.toString()}`,
      );
  ...
  const handleRecharge = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        alert(
          `✅ Successfully recharged with ${selectedPackage.messageCount} messages!`,
        );
  ...
  const handleSendTestMessage = async () => {
    ...
    try {
      ...
      const response = await whatsappApi.post(`api/v1/whatsapp/send-test`, {
        ...
        message:
          "✅ Test message from Denstack! Your WALOCAL integration is working!",
      });
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      alert(
        "File type not supported. Please upload PDF, DOC, DOCX, XLS, XLSX, TXT, ODT, ODS, or RTF files.",
      );
      return;
    }
  ...
  const handleDisconnectWhatsApp = async () => {
    if (
      !window.confirm(
        "Are you sure you want to disconnect WhatsApp? This will remove your settings.",
      )
    ) {
      return;
    }
    ...
    try {
      setIsLoading(true);
      const response = await whatsappApi.post(
        `api/v1/whatsapp/disconnect/${userId}`,
      );
  ...
  const handleUpdateItem = async () => {
    ...
    try {
      ...
      const response = await api.put(
        `${activeTab.apiEndpoint}/${editingProcedure.id}`,
        requestData,
      );
  ...
  const handleDeleteItem = async (id: string) => {
    ...
    if (
      window.confirm(`Are you sure you want to delete this ${itemTypeName}?`)
    ) {
      try {
  ...
  const handleEditItem = async (item: ProcedureItem) => {
    try {
      ...
      alert(
        error.response?.data?.message || "Failed to load item for editing.",
      );
    }
  };
...
const handleSaveColors = async () => {
  ...
  const handleAddService = () => {
    if (
      serviceInput.trim() &&
      !labForm.services.includes(serviceInput.trim())
    ) {
      setLabForm({
        ...labForm,
        services: [...labForm.services, serviceInput.trim()],
      });
      setServiceInput("");
    }
  };
  ...
  const handleRemoveService = (service: string) => {
    setLabForm({
      ...labForm,
      services: labForm.services.filter((s) => s !== service),
    });
  };
  ...
  const handleRegisterLab = async () => {
    ...
    try {
      ...
      const response = await api.post(
        `${labBaseUrl}api/v1/lab/create-inhouse-vendor`,
        {
          name: labForm.name,
          contactPerson: labForm.contactPerson || undefined,
          email: labForm.email || undefined,
      ...
      if (response.data.message) {
        ...
        resetLabForm();
        // Optionally fetch lab list if you implement that endpoint
        // fetchLabVendors();
      }
    } catch (error: any) {
      console.error("Error registering lab:", error);
      alert(error.response?.data?.message || "Failed to register lab");
    } finally {
  ...
  const settingsCards = [
    ...
    },
    {
      ...
      description: "Register and manage in-house laboratory services",
      onClick: () => {
        setShowLabModal(true);
        setShowLabList(false); // Start with form view
      },
    },
  ];
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-green-800 font-medium">
                    Your WALOCAL account is ready!
                  </p>
                  <p className="text-green-600 text-sm">
                    Phone: +91 90740 85694 • Phone ID: 1003912882796252 •
                    Status: CONNECTED • Quality: GREEN
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-gray-800">
                    WALOCAL Message Credits
                  </h3>
                  <p className="text-sm text-gray-600">
                    Tier: {whatsappSettings.messagingTier} • Quality:{" "}
              ...
              <div className="flex items-center gap-6">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-800">
                    {whatsappSettings.messagesUsed}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Remaining</p>
                  <p
                    className={`text-2xl font-bold ${whatsappSettings.messagesRemaining <= 100 ? "text-red-600" : "text-green-600"}`}
                  >
                    {whatsappSettings.messagesRemaining}
                  </p>
                </div>
            ...
            <div className="mt-4">
              <div className="w-full bg-gray-200 rounded-full h-2.5">
                <div
                  ...
                  style={{
                    width: `${(whatsappSettings.messagesUsed / whatsappSettings.messageLimit) * 100}%`,
                  }}
                ></div>
              </div>
              <p className="text-xs text-gray-500 mt-2">
                {whatsappSettings.messagesRemaining} messages remaining today
                (resets at midnight)
              </p>
            </div>
            ...
            {verificationMessage && (
              <div
                ...
                style={{
                  color: verificationMessage.includes("✅")
                    ? "#10b981"
                    : "#ef4444",
                }}
              >
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center">
                    ...
                    <p className="text-sm text-gray-500 mt-2">
                      Supported: PDF, DOC, DOCX, XLS, XLSX, TXT, ODT, ODS, RTF
                      (Max 100MB)
                    </p>
                  </div>
                </div>
                ...
                <div className="flex gap-3">
                  ...
                  >
                    {isLoading
                      ? "Sending..."
                      : `Send Document (${whatsappSettings.messagesRemaining} credits left)`}
                  </button>
                </div>
              </div>
            </div>
          ...
          {!showMessageHistory ? (
            <div className="bg-white rounded-xl shadow-sm p-6">
              {!whatsappSettings.isEnabled ? (
                <div>
                  <h3 className="text-lg font-semibold text-gray-800 mb-4">
                    Activate Your WALOCAL Connection
                  </h3>
                  <p className="text-sm text-gray-600 mb-6">
                    Your WALOCAL account is ready. Just click activate to start
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      />
                      <p className="text-xs text-gray-500 mt-1">
                        Your WALOCAL Phone Number ID (e.g., 1003912882796252)
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      />
                      <p className="text-xs text-gray-500 mt-1">
                        Leave empty to use default WALOCAL API key
                      </p>
                    </div>
                    ...
                    <div className="flex gap-3">
                      <button
                        ...
                        disabled={
                          isVerifying || !whatsappSettings.phoneNumberId
                        }
                    ...
                    {messageHistory.length > 0 ? (
                      ...
                      <tr>
                        ...
                        >
                          ...
                          <p className="text-sm">
                            Send a test message to see history
                          </p>
                        </td>
                      </tr>
                    )}
                ...
                <div className="px-6 py-3 border-t border-gray-200 bg-gray-50">
                  <div className="flex items-center justify-between">
                    <p className="text-sm text-gray-700">
                      Showing {messageHistory.length} of{" "}
                      {historyPagination.totalItems} messages
                    </p>
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => {
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              <div className="text-xs md:text-sm text-gray-500 whitespace-nowrap">
                {isLoading ? (
                  ...
                  <>
                    Total {activeTab.title.toLowerCase()}:{" "}
              ...
              {isLoading && (
                <div className="text-center py-12">
                  ...
                  <p className="mt-2 text-gray-600">
                    Loading {activeTab.title.toLowerCase()}...
                  </p>
                </div>
              )}
                    ...
                    <table className="w-full divide-y divide-gray-200">
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {filteredItems.length > 0 ? (
                          ...
                          <tr>
                            <td
                              colSpan={activeTab.hasPrice ? 5 : 4}
                              className="px-4 py-12 text-center"
                            >
                              <div className="flex flex-col items-center justify-center text-gray-500">
                                <FileText className="w-12 h-12 mb-3 text-gray-300" />
                                <p className="text-lg font-medium mb-1">
                                  No {activeTab.title.toLowerCase()} found
                                </p>
                                <p className="text-sm">
                                  {searchQuery
                                    ? "Try a different search term"
                                    : `No ${itemTypeName}s added yet`}
                                </p>
                                <button
                                  onClick={() => {
                                    resetForm();
            ...
            <div style={{ padding: "1.5rem" }}>
              ...
              >
                ...
                <span className="font-semibold text-gray-900">
                  {whatsappSettings.messagesRemaining} messages
                </span>
              </p>
                ...
                {rechargePackages.map((pkg) => (
                  <div
                    ...
                    style={{
                      ...
                      position: "relative",
                      backgroundColor:
                        selectedPackage?.id === pkg.id ? "#f0fdf4" : "white",
                    }}
                  >
                    {pkg.popular && (
                      <span
                ...
                <div>
                  ...
                  >
                    <input
                      ...
                      onChange={(e) =>
                        handleColorChange("endColor", e.target.value)
                      }
                      style={{
                        width: "3.5rem",
                        height: "3rem",
                ...
                <div>
                  ...
                  >
                    <input
                      ...
                      onChange={(e) =>
                        handleColorChange("sidebarForeground", e.target.value)
                      }
                      style={{
                        width: "3.5rem",
                        height: "3rem",
            ...
            >
              ...
              <div>
                ...
                >
                  ...
                  >
                    {(["city", "state", "country", "zip"] as const).map(
                      (field) => (
                        <input
                          key={field}
                          type="text"
              ...
              >
                ...
                >
                  Patients who rate 4–5 ⭐ will be redirected to your Google
                  review page.{" "}
                  <a
              ...
              >
                <div
                  style={{
                    padding: "0.5rem",
                    background:
                      "linear-gradient(to bottom right, #06b6d4, #0284c7)",
                    borderRadius: "0.5rem",
                  }}
                >
                  <FlaskConical
                ...
                <div>
                  ...
                  >
                    Add and manage your laboratory services
                  </p>
                </div>
              </div>
              <button
            ...
            >
              <div style={{ display: "flex", gap: "1rem" }}>
                <button
                  ...
                  style={{
                    padding: "0.5rem 1rem",
                    borderBottom: !showLabList
                      ? "2px solid #06b6d4"
                      : "2px solid transparent",
                    color: !showLabList ? "#06b6d4" : "#6b7280",
                    fontWeight: 500,
                    fontSize: "0.875rem",
                    background: "none",
                    border: "none",
                ...
                >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={labForm.email}
                      onChange={(e) =>
                        setLabForm({ ...labForm, email: e.target.value })
                      }
                  ...
                  <div>
                    ...
                    >
                      <input
                        ...
                        placeholder="Enter a service (e.g., Blood Test, X-Ray)"
                        style={{
                          flex: 1,
                          padding: "0.5rem 0.75rem",
                          border: "1px solid #d1d5db",
                    ...
                    >
                      {labForm.services.map((service, index) => (
                        <span
                          key={index}
                          style={{
                            display: "inline-flex",
                  ...
                  <div>
                    ...
                    >
                      <input
                        ...
                        onChange={(e) =>
                          setLabForm({ ...labForm, isActive: e.target.checked })
                        }
                        style={{
                          width: "1rem",
                          height: "1rem",
                      ...
                      >
                        Active (Lab is ready to accept requests)
                      </span>
                    </label>
                  </div>
                  ...
                  >
                    ...
                    >
                      ...
                      This lab will be registered as an in-house facility and
                ...
                <div>
                  {isLabLoading ? (
                          ...
                          >
                            ...
                            <span
                              style={{
                                ...
                                fontSize: "0.75rem",
                                fontWeight: 500,
                                backgroundColor: lab.isActive
                                  ? "#dcfce7"
                                  : "#fee2e2",
                                color: lab.isActive ? "#166534" : "#991b1b",
                              }}
                            >
                    ...
                    <div style={{ textAlign: "center", padding: "2rem" }}>
                      ...
                      >
                        Register your first in-house lab to get started
                      </p>
                      <button
                        onClick={() => setShowLabList(false)}
                        style={{
            ...
            {!showLabList && (
              ...
              >
                ...
                <button
                  ...
                  disabled={!labForm.name.trim() || isLabLoading}
                  style={{
                    flex: 1,
                    padding: "0.625rem 1rem",
                    border: "none",
                    borderRadius: "0.375rem",
                    background:
                      !labForm.name.trim() || isLabLoading
                        ? "#9ca3af"
                        : "linear-gradient(to right, #06b6d4, #0284c7)",
                    color: "white",
                    fontSize: "0.875rem",
                  ...
                  onMouseOver={(e) => {
                    if (!(!labForm.name.trim() || isLabLoading)) {
                      e.currentTarget.style.background =
                        "linear-gradient(to right, #0891b2, #075985)";
                    }
                  }}
                  onMouseOut={(e) => {
                    if (!(!labForm.name.trim() || isLabLoading)) {
                ...
                >
                  {isLabLoading ? "Registering..." : "Register Lab"}
                </button>
              </div>
            )}
          </div>
