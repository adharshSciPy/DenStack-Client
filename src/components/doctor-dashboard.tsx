...
const DoctorRegistrationForm: React.FC = () => {
  ...
  const handleAddSpecialization = (e: React.KeyboardEvent<HTMLInputElement>): void => {
  ...
  const validateForm = (): boolean => {
    ...
    } else if (isNaN(Number(formData.standardConsultationFee)) || Number(formData.standardConsultationFee) <= 0) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-blue-50 py-12 px-4">
      <div className="max-w-2xl mx-auto">
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
          {/* Header */}
          <div className="bg-primary p-8 text-white" style={{borderRadius:"20px"}}>
            <h1 className="text-3xl font-bold mb-2">Doctor Registration</h1>
            <p className="text-white/90">Please fill in your details to complete registration</p>
          </div>
          ...
          <div className="p-8 space-y-6">
            ...
            <div className="space-y-2">
              <label htmlFor="doctorUniqueId" className="block text-sm font-medium text-gray-700">
                Doctor Unique ID
              </label>
              <input
                type="text"
            ...
            {/* Role in Clinic */}
            <div className="space-y-2">
              <label htmlFor="roleInClinic" className="block text-sm font-medium text-gray-700">
                Role in Clinic
              </label>
              <select
                id="roleInClinic"
            ...
            {/* Clinic Password */}
            <div className="space-y-2">
              <label htmlFor="clinicPassword" className="block text-sm font-medium text-gray-700">
                Clinic Password
              </label>
            ...
            <div className="space-y-2">
              <label htmlFor="standardConsultationFee" className="block text-sm font-medium text-gray-700">
                Standard Consultation Fee (₹)
              </label>
              <input
                type="number"
              ...
              {errors.standardConsultationFee && (
                <p className="text-sm text-red-600">{errors.standardConsultationFee}</p>
              )}
            </div>

            {/* Specialization */}
            <div className="space-y-3">
              <label htmlFor="specializationInput" className="block text-sm font-medium text-gray-700">
                Specialization
              </label>
              <div className="flex gap-2">
                <input
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSpecializationInput(e.target.value)}
              ...
              <p className="text-xs text-gray-500">Type and click Add button or press Enter</p>
              
              {/* Tags Display */}
              {formData.specialization.length > 0 && (
                <div className="flex flex-wrap gap-2 p-3 bg-gray-50 rounded-lg border border-gray-200">
                  {formData.specialization.map((spec: string, index: number) => (
                    <div
                      key={index}
            ...
            <button
              ...
              className="w-full bg-gradient-to-r from-primary to-secondary text-white py-4 rounded-lg font-semibold text-lg shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200"
            >
